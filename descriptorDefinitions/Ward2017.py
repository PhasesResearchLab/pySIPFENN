...
def local_env_function(local_env, site, element_dict):
  ...
  return [np.concatenate(([eff_coord_num, blen_average, blen_var, volume, sphere_volume], elemental_properties_attributes[0])), 
...
def generate_voronoi_attributes(struct, local_funct=local_env_function):
  ...
  return np.array([value[0] for value in attribute_list]), np.array([value[1] for value in attribute_list]), {i:value[2] for (i, value) in enumerate(attribute_list)}
...
def generate_WC_attributes(strc, neighbor_dict_raw, levels):
  ...
  neighbor_dict = {key: [[strc_unit_cell.index(value[0].to_unit_cell()), value[1]] for value in sp_list] for (key, sp_list) in neighbor_dict_raw.items()}
          ...
          if n_idx[0] != p_idx or abs(n_idx[1] - face_area) > 1e-9 or skipped:
            ...
            if shell < levels - 1:
              if (idx, n_idx[0], n_idx[1]) in reachable_inds[site_idx]:
                reachable_inds[site_idx][(idx, n_idx[0], n_idx[1])] += n_idx[1] * weight / weight_normalization
              else:
                reachable_inds[site_idx][(idx, n_idx[0], n_idx[1])] = n_idx[1] * weight / weight_normalization
          else:
            skipped = True
...
def generate_descriptor(struct):
  ...
  position = 126
  for p in [10,7,5,3,2]:
    properties = np.insert(properties, position, math.pow(sum(math.pow(value, p) for value in element_dict.values()), 1.0 / p))
